...
def laser_Callback(msg):    
    ...
    for range in msg.ranges:
      ...
      if  ang>=180-float(angle)/2 and ang <= 180+float(angle)/2:
            global blocked
            # print(ang)
            # r=Float32()
            # r.data=5.0
...
distance=rospy.get_param("~distance", default="400")
delay=rospy.get_param("~delay", default="15")
angle=rospy.get_param("~angle", default="15")
    
rospy.loginfo("Distance Param == "+str(distance))
rospy.loginfo("Delay Param == "+str(delay))
rospy.loginfo("Angle Param == "+str(angle))   



try:
  rospy.init_node('evitementStm', anonymous=True)
  laser_sub=rospy.Subscriber("/scan",LaserScan,laser_Callback,queue_size=1000)
  true_sub=rospy.Subscriber("/evitement",Bool,true_Callback,queue_size=100)

  currentGoal_sub=rospy.Subscriber("/currentGoal",Point,currentGoal_callback,queue_size=100)
#   pose_sub=rospy.Subscriber("/ta7wil",ta7wil,pose_callback,queue_size=10000)
  evitement_publisher=rospy.Publisher("/evitement",Bool,queue_size=100)
  evitement_cmd=rospy.Publisher("/evitementCmd",Point,queue_size=100)
#   inGo=rospy.Publisher("/ingo",Bool,queue_size=100)
  sending_pub=rospy.Publisher("/sending",Bool,queue_size=100)
  sending_sub=rospy.Subscriber("/sending",Bool,sending_callback,queue_size=100)
  
  
  
  rospy.wait_for_service('/StmPose')
  StmPoseSrv = rospy.ServiceProxy('/StmPose',GetLightProperties)
#   print("done")
