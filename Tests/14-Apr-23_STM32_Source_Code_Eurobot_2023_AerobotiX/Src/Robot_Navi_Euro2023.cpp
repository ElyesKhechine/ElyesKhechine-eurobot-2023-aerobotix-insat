...
int PWM_LB,PWM_RB;
int coef_correct_angle=200; //50 Correction rotate
int right_correction=0,left_correction=0;

//Speed Regulation
float speed_ref, ramp = 400, rampR=35,rampC=500;//max 1500 //deja 800 vitesse kbira
int sens;
double right_error=0,i_right_error=0;
double left_error=0,i_left_error=0;
float kp = 10, ki = 1.8; //kp=10 ki=1.15

//Move
int coef_correct_dist = 20;//25
...
{	int y;
	...
	while(((fabs(total_right-distance)>2)||(fabs(total_left-distance)>2))&& (speed*y/500)<((fabs(distance)*360/1000)+50))
	{  y++;
...
{int z=0;
	...
	while(((fabs(total_right-(goal_distance/2))>2)||(fabs(total_left-(goal_distance/2))>2))&& (speed/500)*z<((fabs(angle)/90)*50) && evitementFlag )
	{   z++;
//		nh.spinOnce();
		t0=t;
		//Accel/Decel Speed Set
