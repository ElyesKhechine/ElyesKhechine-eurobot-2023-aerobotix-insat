...
void run_motors(void)
{
	if (PWM_R>0)
	{
		...
		__HAL_TIM_SetCompare(htim_Motors, right_forward,abs( PWM_R));
	...
	else
	{
		...
		__HAL_TIM_SetCompare(htim_Motors, right_backward, abs( PWM_R));
	}
	if (PWM_L>0)
	{
		if (PWM_L>PWM_Max) PWM_L=PWM_Max;
		__HAL_TIM_SetCompare(htim_Motors, left_forward, abs(PWM_L));
	...
	else
	{
		...
		__HAL_TIM_SetCompare(htim_Motors, left_backward,abs(PWM_L));
	}
