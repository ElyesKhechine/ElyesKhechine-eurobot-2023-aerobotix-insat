...
3 distances (acceleration,constant,deceleration )
ramp aug --> acceleration distance tesghar --> acceleration kesha (ntart) 
wl 3aks bl 3aks 
...
{       
	...
	init();
        // ken distance asgher ml distance elli lezmou yacceleri feha ywali yacceleri w ydeceleri khw 
	...
	while(fabs((total_right+total_left)/2-distance)>1)  // 1mm tolerance (mm maghchouch) 
	{
		t0=t; // compteur baaed kol 10 fois yehseb vitesse 
		 
		if (((total_right+total_left)/2 -distance)<0)
                        // distance parcourue < goal distance -----> mezel lzm nmchiw 9odem
			sens = 1;
		else
                        
                        // distance parcourue > goal distance ----> lzm nwakhrou 
			sens = -1;
		if (fabs((total_right+total_left)/2) < accel_dist) 
                        
                        // distance parcourue < acceleration distance ------> mezel lzm nacceliriw 
			speed_ref = sens*50+sens*(constrain(sqrt (ramp*fabs(total_right+total_left))-50,0,1000));
                         ...
                         // distance parcourue - goal distance < deceleration distance ------> lzm ndeceliriw  
...
void Calcul_Curv(float x, float y,float phi_target) {
	...
	//Calcul des coordonn�es du centre de la courbure
		
	if ((phi_target==0) || (phi_target==180) || (phi_target==-180)){ 
		if ((y==0) && (current_y==0))
				current_y=0.000000001; //R�solution du probleme de division par zero 
		Xc=x;
...
//curv function used in the robot_locate_multi_curv function
 void Multi_Curv(float R,float theta ,float speed,int i,int n){ 
	initMultiCurv(); // integrale accumul�e
	theta=(theta*PI)/180;
	goalC=fabs(R*theta); 
	remain_distC=goalC-fabs(total_centre);
	speedC=speed;
	if (goalC < (speedC*speedC/rampC)) {
		...
		if (i!=0) accel_dist=0; //Acc�l�ration du robot juste au d�but de la trajectoire
		if (i!=(n-1)) decel_dist=0;// D�c�l�ration du robot seulement a la fin de la trajectoire
...
void rotate(float angle, float speed)
{
	init();
	//Set accel/decel distance
	goal_distance = angle * PI * spacing_encoder/ 180; // calcul de arc avec angle en rad 
