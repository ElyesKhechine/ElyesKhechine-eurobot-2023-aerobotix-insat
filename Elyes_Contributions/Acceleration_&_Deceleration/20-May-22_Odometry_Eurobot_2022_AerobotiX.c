...
volatile double right_speed = 0,right_encoder_speed = 0;
volatile int robot_speed = 0;

TIM_HandleTypeDef* htim_left_Encoder;
TIM_TypeDef* left_TIM;
double left_resolution;
int left_precision;
int left_sens;
...
volatile double left_speed = 0,left_encoder_speed = 0;
...
volatile double phi_speed,d_phi_counter,phi_speed_deg;
volatile float current_phi_deg;

float ref_x,ref_y,dec=100;

void set_right_encoder(TIM_HandleTypeDef* htim, TIM_TypeDef* TIM, double resolution, int precision, int sens)
{
...
void set_left_encoder(TIM_HandleTypeDef* htim, TIM_TypeDef* TIM, double resolution, int precision, int sens)
{
...
void speed_calcul(void)
{
	...
	phi_speed_deg = rad_to_deg(phi_speed);
	d_phi_counter = 0;
	...
	robot_speed = (int)(right_speed + left_speed) / 2;
}
...
float ticks_to_distance(int ticks, float rayon, int resolution, int precision) 
{
	return (ticks*2*PI*rayon/(resolution*precision));
}
