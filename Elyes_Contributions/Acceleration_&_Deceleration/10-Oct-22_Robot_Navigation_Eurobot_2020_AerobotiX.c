...
void first_go_non_stop(float goal_x, float goal_y,float vitesse_max_mm)
{
	...
	if (fabs(distance_mm) < (0.5*acceleration_mm*temps_rampe_s_accel*temps_rampe_s_accel))
	{
...
void Robot_Locate_nouveau(float goal_x, float goal_y, float speed)
{
	...
	move_distance_asservi(goal_x,goal_y,speed);
}
...
void move(float speed, int delay)
{
	...
	while (t<delay)
	{
		...
		right_error = speed_ref - right_speed;
		i_right_error += right_error;
		PWM_R = kp_right * right_error + ki_right * i_right_error;
		//Left wheel regulation
		left_error = speed_ref - left_speed;
		i_left_error += left_error;
		PWM_L = kp_left * left_error + ki_left * i_left_error;
		run_motors();
		do delta=t-t0;
		while (delta<T);
	}
