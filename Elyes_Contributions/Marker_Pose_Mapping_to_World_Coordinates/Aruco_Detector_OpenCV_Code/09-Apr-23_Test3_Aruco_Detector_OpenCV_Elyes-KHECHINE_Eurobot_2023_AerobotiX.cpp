...
void callback(const ImageConstPtr &image_msg)
{
    ...
    std::vector<std::vector<geometry_msgs::Point>> all_corners;
    for (int i = 0; i < ids.size(); i++)
    {   if (ids[i]==47){ar_msg.marker_ids.push_back(ids[i]*10+rose--);}
        if (ids[i]==36){ar_msg.marker_ids.push_back(ids[i]*10+marron--);}
        if (ids[i]==13){ar_msg.marker_ids.push_back(ids[i]*10+jaune--);}
        ...
        std::vector<geometry_msgs::Point> corners1;
        for (int i = 0; i < 4; i++) {
        geometry_msgs::Point corner;
        corner.x = one_corner[i].x;
        corner.y = one_corner[i].y;
        corners1.push_back(corner);
        }

        // Add the corners vector to the all_corners vector
        all_corners.push_back(corners1);
    }
    ar_msg.marker_corners = all_corners;
    ...
    if (ids.size() > 0)
    {
        ...
        if (result_img_pub_.getNumSubscribers() > 0)
        {
            ...
            {    
                ...
                vector<Point2f> one_corner = corners[i];
               int x=(one_corner[0].x + one_corner[1].x + one_corner[2].x + one_corner[3].x) / 4;
               int y=(one_corner[0].y + one_corner[1].y + one_corner[2].y + one_corner[3].y) / 4;
                ...
                if (prec >= min_prec_value)
                {
                    ...
                    cv::putText(display_image, "id: " + SSTR(ids[i]) + " z dis: " + SSTR(distance_z) + " m  " + SSTR(ROUND2(prec)) + " %"+" x: "+SSTR(x)+" y: "+SSTR(y), cv::Point(10, 70 + i * 30), cv::FONT_HERSHEY_SIMPLEX, 0.9, CV_RGB(0, 255, 0), 2);
                    result_img_pub_.publish(cv_bridge::CvImage(std_msgs::Header(), "bgr8", display_image).toImageMsg());
                }
            }
        }
        ...
        int arucoRedCount = 0;
        int arucoBlueCount = 0;
        int arucoGreenCount = 0;
        int arucoColorlessCount = 0;
        int arucoMilieu = 0;
        int arucoCalibration = 0;
        ...
        if (arucoRedCount > 1)
        {
            for (auto i = 0; i < rotation_vectors.size(); ++i)
            {
                if (getPrec(ids, i) > min_prec_value && ids[i] == 47)
                {
                    ...
                    tf_msg.child_frame_id = ss.str() + " " + to_string(arucoRedCount--) + role;
        ...
        else
        {
            for (auto i = 0; i < rotation_vectors.size(); ++i)
            {
                if (getPrec(ids, i) > min_prec_value && ids[i] == 47)
                {
                    ...
                    stringstream ss;
                    ss << marker_tf_prefix << ids[i];
                    tf_msg.child_frame_id = ss.str()+role;
                    tf_msg.transform.translation.x = transform.getOrigin().getX();
                    tf_msg.transform.translation.y = transform.getOrigin().getY();
                    tf_msg.transform.translation.z = transform.getOrigin().getZ();
                    tf_msg.transform.rotation.x = transform.getRotation().getX();
        ...
        if (arucoBlueCount > 1)
        {
            for (auto i = 0; i < rotation_vectors.size(); ++i)
            {
                if (getPrec(ids, i) > min_prec_value && ids[i] == 13)
                {
                    // ROS_INFO("aruco markers tf");
                    auto translation_vector = translation_vectors[i];
                    auto rotation_vector = rotation_vectors[i];
                    ...
                    stringstream ss;
                    ss << marker_tf_prefix << ids[i];
                    tf_msg.child_frame_id = ss.str() + to_string(arucoBlueCount--) + role;
                    tf_msg.transform.translation.x = transform.getOrigin().getX();
                    tf_msg.transform.translation.y = transform.getOrigin().getY();
                    tf_msg.transform.translation.z = transform.getOrigin().getZ();
                    tf_msg.transform.rotation.x = transform.getRotation().getX();
        ...
        if (arucoGreenCount > 1)
        {
            for (auto i = 0; i < rotation_vectors.size(); ++i)
            {
                if (getPrec(ids, i) > min_prec_value && ids[i] == 36)
                {
                    // ROS_INFO("aruco markers tf");
                    auto translation_vector = translation_vectors[i];
                    auto rotation_vector = rotation_vectors[i];
                    ...
                    stringstream ss;
                    ss << marker_tf_prefix << ids[i];
                    tf_msg.child_frame_id = ss.str() + " " + to_string(arucoGreenCount--) + role;
                    tf_msg.transform.translation.x = transform.getOrigin().getX();
                    tf_msg.transform.translation.y = transform.getOrigin().getY();
                    tf_msg.transform.translation.z = transform.getOrigin().getZ();
                    tf_msg.transform.rotation.x = transform.getRotation().getX();
        ...
        if (arucoColorlessCount > 1)
        {
            for (auto i = 0; i < rotation_vectors.size(); ++i)
            {
                if (getPrec(ids, i) > min_prec_value && ids[i] == 17)
                {
                    // ROS_INFO("aruco markers tf");
                    auto translation_vector = translation_vectors[i];
                    auto rotation_vector = rotation_vectors[i];
                    ...
                    stringstream ss;
                    ss << marker_tf_prefix << ids[i];
                    tf_msg.child_frame_id = ss.str() + +" " + to_string(arucoColorlessCount--) + role;
                    tf_msg.transform.translation.x = transform.getOrigin().getX();
                    tf_msg.transform.translation.y = transform.getOrigin().getY();
                    tf_msg.transform.translation.z = transform.getOrigin().getZ();
                    tf_msg.transform.rotation.x = transform.getRotation().getX();
        ...
        if (arucoMilieu > 1)
        {
            for (auto i = 0; i < rotation_vectors.size(); ++i)
            {
                if (getPrec(ids, i) > min_prec_value && ids[i] == 42)
                {
                    // ROS_INFO("aruco markers tf");
                    auto translation_vector = translation_vectors[i];
                    auto rotation_vector = rotation_vectors[i];
                    ...
                    stringstream ss;
                    ss << marker_tf_prefix << ids[i];
                    tf_msg.child_frame_id = ss.str() + " " + to_string(arucoMilieu--) + role;
                    tf_msg.transform.translation.x = transform.getOrigin().getX();
                    tf_msg.transform.translation.y = transform.getOrigin().getY();
                    tf_msg.transform.translation.z = transform.getOrigin().getZ();
                    tf_msg.transform.rotation.x = transform.getRotation().getX();
        ...
        if (arucoCalibration > 1)
        {
            for (auto i = 0; i < rotation_vectors.size(); ++i)
            {
                if (getPrec(ids, i) > min_prec_value && ids[i] == 1)
                {
                    // ROS_INFO("aruco markers tf");
                    auto translation_vector = translation_vectors[i];
                    auto rotation_vector = rotation_vectors[i];
                    ...
                    stringstream ss;
                    ss << marker_tf_prefix << ids[i];
                    tf_msg.child_frame_id = ss.str() + " " + to_string(arucoCalibration--) + role + "/etalon";
                    tf_msg.transform.translation.x = transform.getOrigin().getX();
                    tf_msg.transform.translation.y = transform.getOrigin().getY();
                    tf_msg.transform.translation.z = transform.getOrigin().getZ();
                    tf_msg.transform.rotation.x = transform.getRotation().getX();
        ...
        else
        {
            for (auto i = 0; i < rotation_vectors.size(); ++i)
            {
                if (getPrec(ids, i) > min_prec_value && ids[i] == 1)
                {
                    ...
                    stringstream ss;
                    ss << marker_tf_prefix << ids[i];
                    tf_msg.child_frame_id = ss.str() + role + "/etalon";
                    tf_msg.transform.translation.x = transform.getOrigin().getX();
                    tf_msg.transform.translation.y = transform.getOrigin().getY();
                    tf_msg.transform.translation.z = transform.getOrigin().getZ();
                    tf_msg.transform.rotation.x = transform.getRotation().getX();
...
int main(int argc, char **argv)
{
    ...
    nh.param("dictionary_name", dictionary_name, string("DICT_4X4_50"));
    nh.param("role", role, string(""));
    ...
    result_img_pub_ = it.advertise(rgb_topic + "/result_img", 1);
    tf_list_pub_ = nh.advertise<tf2_msgs::TFMessage>(rgb_topic + "/tf_list", 10);

    aruco_info_pub_ = nh.advertise<alfons_msgs::ArucoInfo>(rgb_topic + "/aruco_list", 10);
